 ...
 *  	BuyerDao buyerDao;
 *  객체의 초기화 : 선언된 객체를 통하여 변수, method 에 접근할 수 있도록 하는 조치
...
public class BuyerServiceImplV1 implements BuyerService {
	...
	// 클래스 영역에 선언된 변수, 객체는 final 키워드를 필수적으로 사용하자
	// final 키워드를 사용하여 선언된 객체는 만약 초기화가 되지 않으면
	// 문법 오류를 보여 반드시 초기화 하도록 독려한다.
	// final 로 선언된 객체는 반드시 현재 클래스의 생성자에서 초기화를 해 주어야 한다.
...
//		System.out.println("고객정보" + buyerDto);
	...
	public BuyerDto findByBuId(String strBuId) {
		...
		try {
			...
			if (buyerDto == null) {
				System.out.printf("없는 고객 정보 입니다.(%s)", strBuId);
			} else {
			...
			HelpMessage.ERROR("고객 ID 는 정수로 입력하세요", String.format("입력한 고객 ID : %s", strBuId));
		}
	...
	public BuyerDto findByBuName() {
		while (true) {
			...
			if (buyerDto == null) {
				HelpMessage.ERROR("고객ID 를 확인하세요");
