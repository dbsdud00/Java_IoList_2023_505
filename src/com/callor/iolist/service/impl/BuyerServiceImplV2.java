...
public class BuyerServiceImplV2 extends BuyerServiceImplV1 {
	// BuyerDao, Sanner 객체를 V1에서 상속받아 선언한 상태
	...
	public BuyerDto findByBuId() {
		while (true) {
			...
			if (buyerDto == null) {
				HelpMessage.ERROR(String.format("입력한 ID 의 고객정보가 없습니다.(%s)", strBuId));
				continue;
			}
			return buyerDto;
	...
	protected String inputBuId() {
		while (true) {
			...
			if (strBuId.equals("QUIT"))
				return strBuId;
			if (strBuId.isBlank()) {
				HelpMessage.CONFIRM("고객ID가 없습니다", "고객 ID 를 새로 생성할 까요?(Y:생성)");
	...
	public void insert() {
		...
		while (true) {
			...
			if (buyerDto.buName == null && strName.isEmpty()) {
				HelpMessage.ERROR("고객이름은 반드시 입력해야 한다.");
				continue;
		...
		HelpMessage.ALERT("입력한 고객 이름 확인 : " + buyerDto.buTel);
		...
		while (true) {
			...
			if (buyerDto.buTel == null && strTel.isEmpty()) {
				HelpMessage.ERROR("전화번호는 반드시 입력해야 한다.");
				continue;
		...
		while (true) {
			...
			if (buyerDto.buAddr == null && strAddr.isEmpty()) {
				HelpMessage.ERROR("주소는 반드시 입력해야 한다.");
				continue;
